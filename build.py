...
SECTIONS = [
    ("ポケットカード", "ベッドサイドで開く要点まとめ", [
        ...
        ("legionella.html",
         ...
         "感染症・呼吸"),
        ("sls-tdp.html",
         "Short-Long-Short配列ポケットカード.html",
         "Short-Long-Short 配列",
         "pause依存性TdPの開始様式。治療の主眼は薬でなくpauseを消すこと。SLSの有無で治療が正反対に分岐する。",
         "不整脈・心電図"),
        ("gemella.html",
         "Gemella菌血症ポケットカード.html",
         "Gemella 菌血症",
        ...
        ("antiarrhythmics.html",
         "抗不整脈薬ポケットカード.html",
         "抗不整脈薬 分類と使い方",
         "Ⅰ群はNa遮断、Ⅰaは＋K遮断、Ⅰbは不活性化型選択、ⅠcはK遮断なし。止まったら中止し、Na遮断は持続せず、K遮断は持続してよい。",
         "不整脈・薬剤"),
        ("antiarrhythmics-dosing.html",
         "抗不整脈薬 投与法ポケットカード.html",
         "抗不整脈薬 静注の実際",
         "1Aを生食20mLで5分iv、または生食100mLで30分〜1時間DIV。持続してよいのはⅢ群とリドカインだけ。1A固定は体重50kg分。",
         "不整脈・薬剤"),
        ("preop-check.html",
         "術前確認ポケットカード.html",
         "術前確認 手術に出す前に",
         "最初に決めるのは「あと何時間待てるか」。低リスクのルーチン検査は転帰を変えないが、薬・気道と胃・出血への備え・持ち込み抗菌薬の抜けは直接命に関わる。",
         "周術期・術前"),
    ]),
    ("アプリ", "その場で計算・練習するもの", [
